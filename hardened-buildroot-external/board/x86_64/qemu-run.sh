...
DISK_IMG="$(realpath "${CWD}"/../../output/x86_64/images/disk.img)"
...
check_env() {
  ...
  if [[ ! -e "${DISK_IMG}" ]]; then
    echo "CRITICAL: ${DISK_IMG} does not exit. Please build the x86_64_defconfig image!"
    exit 1
  fi
}
...
run_virt() {
  ...
  qemu-system-x86_64 \
    ...
    -smbios type=2 \
    -drive file="${DISK_IMG}",format=raw,id=pcu,if=virtio \
